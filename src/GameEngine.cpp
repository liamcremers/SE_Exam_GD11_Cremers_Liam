...
bool GameEngine::SetWindowRegion(const HitRegion* regionPtr)
{
    ...
    else
    {
        ...
        m_WindowRegionPtr->Move(GetSystemMetrics(SM_CXFIXEDFRAME),
...
void GameEngine::SetKeyList(const tstring& keyList)
{
    ...
    if (keyList.size() > 0)
    {
        ...
        for (int count{}; count < (int)keyList.size() + 1; ++count)
        {
            ...
            m_KeyListPtr[count] = (key > 96 && key < 123) ? key - 32 : key;
            // insert the key, coverted to uppercase if supplied character is lowercase
        }
    }
}
...
bool GameEngine::DrawLine(int x1, int y1, int x2, int y2) const
{
    if (m_IsPainting)
    {
        ...
        MoveToEx(m_HdcDraw, 0, 0, nullptr);
        // reset the position - sees to it that eg. AngleArc draws from 0,0 instead of the last position of DrawLine
...
void GameEngine::FormPolygon(const POINT ptsArr[], int count, bool close) const
{
    ...
    {
        POINT* newPtsArr = new POINT[count + 1];
        // interesting case: this code does not work with memory allocation at compile time => demo case for dynamic memory use
...
bool GameEngine::FillRect(int left, int top, int right, int bottom, int opacity) const
{
    if (m_IsPainting)
    {
        ...
        AlphaBlend(m_HdcDraw, left, top, dim.right, dim.bottom, tempHdc, dim.left, dim.top, dim.right, dim.bottom,
                   blend);
...
POINT GameEngine::AngleToPoint(int left, int top, int right, int bottom, int angle) const
{
    ...
    else
    {
        ...
        if (angle > 90 && angle < 270) pt.x *= -1;
        // sqrt returns the positive value of the square, take the negative value if necessary
...
bool GameEngine::DrawBitmap(const Bitmap* bitmapPtr, int left, int top, RECT rect) const
{
    if (m_IsPainting)
    {
        ...
        if (bitmapPtr->HasAlphaChannel())
        {
            ...
            // blend function combines opacity and pixel based transparency
            AlphaBlend(m_HdcDraw, left, top, rect.right - rect.left, rect.bottom - rect.top, hdcMem, rect.left,
                       rect.top, rect.right - rect.left, rect.bottom - rect.top, blender);
        }
        else
            TransparentBlt(m_HdcDraw, left, top, rect.right - rect.left, rect.bottom - rect.top, hdcMem, rect.left,
                           rect.top, rect.right - rect.left, rect.bottom - rect.top,
                           bitmapPtr->GetTransparencyColor());
...
Bitmap::Bitmap(const tstring& filename, bool createAlphaChannel) : m_HasAlphaChannel(createAlphaChannel)
{
    ...
    {
        m_hBitmap = (HBITMAP)LoadImage(GAME_ENGINE->GetInstance(), filename.c_str(), IMAGE_BITMAP, 0, 0,
                                       LR_LOADFROMFILE);
...
void Bitmap::CreateAlphaChannel()
{
    ...
    GetDIBits(windowDC, m_hBitmap, 0, GetHeight(), m_PixelsPtr, (BITMAPINFO*)&bminfoheader, DIB_RGB_COLORS);
    // load pixel info
...
void Bitmap::SetTransparencyColor(COLORREF color) // converts transparency value to pixel-based alpha
{
    ...
    if (HasAlphaChannel())
    {
        ...
        {
            if (RGB(m_PixelsPtr[count * 4 + 2], m_PixelsPtr[count * 4 + 1], m_PixelsPtr[count * 4]) == color)
            // if the color of this pixel == transparency color
            {
                ((int*)newPixelsPtr)[count] = 0;
                // set all four values to zero, this assumes sizeof(int) == 4 on this system
                // setting values to zero means premultiplying the RGB values to an alpha of 0
            }
            else ((int*)newPixelsPtr)[count] = ((int*)m_PixelsPtr)[count];
            // copy all four values from m_PixelsPtr to NewPixels
        }

        SetDIBits(windowDC, m_hBitmap, 0, height, newPixelsPtr, (BITMAPINFO*)&bminfoheader, DIB_RGB_COLORS);
...
bool Bitmap::SaveToFile(const tstring& filename) const
{
    ...
    HANDLE hFile = ::CreateFile(filename.c_str(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_WRITE, nullptr,
...
{
    // Create the edit box
    m_WndEdit = CreateWindow(_T("EDIT"), text.c_str(),
                             WS_BORDER | WS_CHILD | WS_CLIPSIBLINGS | WS_TABSTOP | ES_LEFT | ES_AUTOHSCROLL, 0, 0, 0, 0,
                             GAME_ENGINE->GetWindow(), NULL, GAME_ENGINE->GetInstance(), nullptr);
...
Button::Button(const tstring& label)
{
    ...
    m_WndButton = CreateWindow(_T("BUTTON"), label.c_str(),
                               WS_BORDER | WS_CHILD | WS_CLIPSIBLINGS | WS_TABSTOP | BS_PUSHBUTTON, 0, 0, 0, 0,
                               GAME_ENGINE->GetWindow(), NULL, GAME_ENGINE->GetInstance(), nullptr);
...
LRESULT Button::ButtonProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_LBUTTONDBLCLK:
        // clicking fast will throw LBUTTONDBLCLK's as well as LBUTTONDOWN's, you need to capture both to catch all button clicks
        m_Armed = true;
        break;
    case WM_LBUTTONUP:
        if (m_Armed)
...
void Timer::Start()
{
    ...
    {
        CreateTimerQueueTimer(&m_TimerHandle, NULL, TimerProcStatic, (void*)this, m_Delay, m_Delay,
...
HRGN HitRegion::BitmapToRegion(HBITMAP hBmp, COLORREF cTransparentColor, COLORREF cTolerance) const
{
    ...
    if (hBmp)
    {
        ...
        if (hMemDC)
        {
            ...
            VOID* pbits32;
            HBITMAP hbm32 = CreateDIBSection(hMemDC, (BITMAPINFO*)&RGB32BitsBitmapInfo, DIB_RGB_COLORS, &pbits32, NULL,
                                             NULL);
